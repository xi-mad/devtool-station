  ...
  return (
    <div className="flex flex-col h-[calc(100vh-180px)] gap-4 transition-colors">
      <div className="bg-white dark:bg-slate-900 p-2 rounded-xl shadow-sm border border-slate-200 dark:border-slate-800 flex flex-wrap gap-2 transition-colors">
        ...
        <CopyButton text={text} className="px-3 py-1.5 text-xs font-medium text-slate-600 dark:text-slate-400 bg-slate-50 dark:bg-slate-800 hover:bg-slate-100 dark:hover:bg-slate-700 hover:text-brand-600 dark:hover:text-brand-400 rounded-lg border border-slate-200 dark:border-slate-700 transition-colors flex items-center gap-1">
          {(copied) => <>{copied ? <Check size={14} /> : <Copy size={14} />}</>}
        </CopyButton>
        <ClearButton onClick={() => setText('')} className="px-3 py-1.5 text-xs font-medium text-slate-600 dark:text-slate-400 bg-slate-50 dark:bg-slate-800 hover:bg-red-50 dark:hover:bg-red-500/10 hover:text-red-600 dark:hover:text-red-400 rounded-lg border border-slate-200 dark:border-slate-700 transition-colors flex items-center gap-1" />
      </div>
      ...
      <div className="flex-1 flex flex-col md:flex-row gap-6 min-h-0">
        ...
        <div className="w-full md:w-64 flex flex-col gap-4">
          <div className="bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-slate-200 dark:border-slate-800 overflow-hidden transition-colors">
            <SectionHeader icon={Hash} title={t('text-inspector.statistics')} />
